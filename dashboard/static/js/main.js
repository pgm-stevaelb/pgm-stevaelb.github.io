...
(() => {
	const app = {
		...
		cacheElem() {
			...
			this.$searchBox = document.querySelector('search-box__input');
		...
		async getGithubUsers() {
			const APIURL = 'https://api.github.com/search/users?sort=desc&page=1&per_page=100&q=pgm-stevaelb';
			try {
				const response = await fetch(APIURL);
				const data = await response.json();
				this.$githubUsers.innerHTML = `
				<div class="search-result">
					<img src="${data.items[0].avatar_url}" alt="${data.items[0].login}" class="search-result__img">
					<p>${data.items[0].login}</p>
				</div>`;
			} catch (error) {
				console.error(error.message);
			}
		...
		searchBox() {
			...
			this.$searchButton.addEventListener("click", () => console.log(this.$searchBox.value), false);
			console.log(url);
		}
	};
